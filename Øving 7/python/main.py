...
for image_number in [4]: # Use this to run on a single image
    ...
    uv1 = np.vstack((uv, np.ones((1, uv.shape[1]))))  
    xy1 = np.linalg.inv(K) @ uv1                       
    xy  = xy1[:2, :] / xy1[2, :]       # TASK: Compute (x,y) as defined in the text
    H = estimate_H(xy, XY[:, valid])   # TASK: Implement this function
    xy_from_H= H@XY1 #eq 7
    uv_from_H1 = K@xy_from_H # TASK: Compute predicted pixel coordinates using H
    uv_from_H  = uv_from_H1[:2, :] / uv_from_H1[2, :] #Dehomoginize

    e = np.linalg.norm(uv - uv_from_H[:, valid], axis=0)    
    print(e)
    print(f"Max error: {e.max()}")
    print(f"Min error: {e.min()}")
